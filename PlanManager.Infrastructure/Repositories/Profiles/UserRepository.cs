...
public class UserRepository : Repository<User>, IUserRepository
{
    ...
    {
        var groupPermission = await _context.Users.Where(x => x.Id == idUser).Select(x => x.IdGroupPermission).FirstOrDefaultAsync();

        var permissions = await _context.Permissions.Where(x => x.IdGroupPermission == groupPermission)
            .Select(x => x.KeyPermission).ToListAsync();
        return permissions;
    }
}