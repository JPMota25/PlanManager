...
public class SignMap : IEntityTypeConfiguration<Sign> {
	public void Configure(EntityTypeBuilder<Sign> builder) {
		...
		builder.HasOne(x => x.Company).WithOne().HasForeignKey<Sign>(x => x.IdCompany).HasConstraintName("FK_Sign_Company_IdCompany").OnDelete(DeleteBehavior.Restrict);

        builder.Property(x => x.IdPlan).HasColumnName("Plan").HasColumnType("nvarchar").HasMaxLength(11).IsRequired();
        builder.HasOne(x => x.Plan).WithMany().HasForeignKey(x => x.IdPlan).HasConstraintName("FK_Sign_Plan");
