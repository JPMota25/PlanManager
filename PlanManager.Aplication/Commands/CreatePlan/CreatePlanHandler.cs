...
public class CreatePlanHandler : Notifiable<Notification>, IRequestHandler<CreatePlanCommand, ResultDto<PlanCreatedDto>> {
	...
	public CreatePlanHandler(IPlanService planService, ILogActivityService logActivityService) {
	...
	public async Task<ResultDto<PlanCreatedDto>> Handle(CreatePlanCommand request, CancellationToken cancellationToken) {
		...
		if (await _planService.GetPlanByName(plan.Name) != null)
		...
		await _logActivityService.CreateLog(ELogType.Success, EAction.Created, ELogCode.CreatePlan, plan.Id, new Description("Plan created succesfully."));
		return ResultDto<PlanCreatedDto>.Ok(new PlanCreatedDto(plan.Id.Identifier, plan.Name.Value, plan.Value.ValueWith2Digits, plan.IdCompany.Identifier));
	}
}