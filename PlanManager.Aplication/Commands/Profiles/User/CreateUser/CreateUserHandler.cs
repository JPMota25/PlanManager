...
public class CreateUserHandler : Notifiable<Notification>, IRequestHandler<CreateUserCommand, ResultDto<ResponseUserCreated>>
{
    ...
    public async Task<ResultDto<ResponseUserCreated>> Handle(CreateUserCommand command, CancellationToken cancellationToken)
    {
        Person request = command.Person;
        if (!request.IsValid)
            return ResultDto<ResponseUserCreated>.Fail(request.Notifications);
        ...
        if (!person.IsValid || await _personService.VerifyPersonUniqueKeys(person))
            return ResultDto<ResponseUserCreated>.Fail(person.Notifications);
        ...
        return ResultDto<ResponseUserCreated>.Ok(new ResponseUserCreated(user.Id, user.Username));
    }
}