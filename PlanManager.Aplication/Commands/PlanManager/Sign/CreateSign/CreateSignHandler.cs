...
public class CreateSignHandler : Notifiable<Notification>, IRequestHandler<CreateSignCommand, ResultDto<ResponseSignCreated>>
{
    ...
    public async Task<ResultDto<ResponseSignCreated>> Handle(CreateSignCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid)
            return ResultDto<ResponseSignCreated>.Fail(request.Notifications);
        ...
        if (await _signService.VerifyIfSignExists(sign))
            return ResultDto<ResponseSignCreated>.Fail(new Notification("Sign.Handler", "Sign already exists"));
        ...
        return ResultDto<ResponseSignCreated>.Ok(new ResponseSignCreated());
    }
}