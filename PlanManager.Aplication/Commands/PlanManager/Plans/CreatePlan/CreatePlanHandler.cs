...
public class CreatePlanHandler : Notifiable<Notification>, IRequestHandler<CreatePlanCommand, ResultDto<ResponsePlanCreated>>
{
    ...
    public async Task<ResultDto<ResponsePlanCreated>> Handle(CreatePlanCommand request, CancellationToken cancellationToken)
    {
        if (!request.IsValid)
            return ResultDto<ResponsePlanCreated>.Fail(request.Notifications);
        ...
        if (await _planService.GetPlanByNameAndCompany(plan.Name, plan.IdCompany) != null)
            return ResultDto<ResponsePlanCreated>.Fail(new Notification("PlanHandler.Plan", "Plan name already exists"));
        ...
        return ResultDto<ResponsePlanCreated>.Ok(new ResponsePlanCreated(plan.Id, plan.Name, plan.Value, plan.IdCompany));
    }
}